...
BATCH_SIZE = 64
DATASET = 'train_image'
CHECKPOINT_DIR_v1 = 'checkpoint_v1'
SAMPLE_DIR = 'samples'
...
class DCGAN(object):
	def __init__(self):
		...
		self.checkpoint_dir = CHECKPOINT_DIR_v1
		self.build()
	...
	def build(self):
		...
		self.d, self.d_logits = self.D(self.images, is_training=self.is_training)
		self.d_, self.d_logits_ = self.D(self.g, is_training=self.is_training)

		self.g_sum = tf.summary.histogram('g', self.g)
		self.d_sum = tf.summary.histogram('d', self.d)
		self.d_sum_ = tf.summary.histogram('d_', self.d_)
		...
		self.d_op = tf.train.AdamOptimizer(self.learning_rate, beta1=BETA1).minimize(self.d_loss, var_list=self.d_vars)
		self.g_op = tf.train.AdamOptimizer(self.learning_rate, beta1=BETA1).minimize(self.g_loss, var_list=self.g_vars)
	...
	def train_gan(self):
		...
		self.summary_op = tf.summary.merge(
			[self.z_sum, self.g_sum, self.d_sum_, self.d_loss_fake_sum, self.g_loss_sum,
		...
		for epoch in range(EPOCH):
			...
			for index in range(batch_index):
				...
				err_d_fake = self.sess.run(
					self.d_loss_fake,
					feed_dict={self.z: train_z, self.is_training: False}
				)
				err_d_real = self.sess.run(
					self.d_loss_real,
					feed_dict={self.images: train_images, self.is_training: False}
				)
				err_g = self.sess.run(
					self.g_loss,
					feed_dict={self.z: train_z, self.is_training: False}
				...
				print('Epoch: [{:2d}] [{:4d}/{:4d}], d_loss: {:.8f}, g_loss: {:.8f}'.format(
					epoch, index, batch_index, err_d_fake + err_d_real, err_g))
